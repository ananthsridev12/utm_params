...
function initials(?string $name): string
{
    ...
    $letters = strtoupper(($parts[0][0] ?? '') . ($parts[count($parts) - 1][0] ?? ''));
    return $letters !== '' ? substr($letters, 0, 2) : '?';
}
...
/**
 * Inline SVG icon set (hand-drawn, Feather-style outline icons -- no icon
 * font or external request). $name keys used across the sidebar nav,
 * dashboard stat cards, and a few action buttons.
 */
function icon(string $name, string $class = ''): string
{
    static $paths = [
        ...
        'browser'      => '<rect x="3" y="3" width="18" height="18" rx="2"></rect><line x1="3" y1="9" x2="21" y2="9"></line><line x1="9" y1="21" x2="9" y2="9"></line>',
    ...
    return '<svg' . $classAttr . ' viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' . $body . '</svg>';
}
